...
with tab1:
    ...
    # 기본 선택값 설정 (index 강제 X)
    if "selected_name" not in st.session_state:
        st.session_state.selected_name = "이태희"  # 기본값

    # 고객 selectbox
    selected_name = st.selectbox(
        "고객명",
        name_list,
        index=name_list.index(st.session_state.selected_name)
        if st.session_state.selected_name in name_list else 0
    )
    ...
    if st.button("조회하기"):
        ...
        """
        st.session_state.search_result = conn.execute(query).df()

    # 조회 결과 표시
    if "search_result" in st.session_state:
        df_show = st.session_state.search_result
        if df_show.empty:
